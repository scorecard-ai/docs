...
<Update label="August 29, 2025">
  ...
  New users now start with a pre-configured example project and automatic first run - see results immediately without setup.
  ...
  New comprehensive guides for previously undocumented features:
  - [Custom Endpoints](/features/endpoints) - Test and evaluate HTTP APIs
  - [Traces & Error Tracking](/features/traces) - Debug and monitor AI applications  
  - [Synthetic Data Generation](/features/synthetic-data-generation) - Generate test data with AI

  ### 🐛 Bug Fixes and Improvements
  
  We fixed schema validation when creating new testcases and enabled real-time status updates on project cards. The model selector scrolling issue in the Kickoff Run modal has been resolved, and empty states now include helpful documentation links. Charts and status properly refresh immediately after creating new runs.
</Update>
