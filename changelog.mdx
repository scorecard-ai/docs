...
# Note: In style.css, there is a rule that applies to pages with title "Product Updates"
...
<Update label="September 11, 2026">
  ### Multiple graders per metric

  Several people can now score the same [metric](/features/metrics) on a record, and each grader's score is kept separately, so one metric can carry independent reviews and you can see where graders disagree. A cell shows one score with a count badge when more than one grader has scored it. Open the card to page through each grader's value and reasoning, or click **Add my score** to add yours. Metric widgets get a grader-scope pill: **All** counts every grader's latest score as a vote, or pick one grader to see only their scores and what they have left to grade.

  {/* TODO(screenshot): /images/changelog-multi-grader-scores-{light,dark}.png — score card popover with the grader pager (1 of 3) and Add my score, plus a metric widget with the grader-scope pill */}

  ### Blind evaluations on the Compare page

  Org members see compared records as **Model A**, **Model B**, and **Model C**, with identity taken from each run's System instead of record tags, so reviewers judge outputs without the system name in front of them. Admins see the System name and version, each grader's pick as an avatar under the option they chose with their reasoning, dashed **awaiting** marks for assigned reviewers who haven't graded yet, and a **Reviewers** column on the record picker with per-reviewer progress.

  {/* TODO(screenshot): /images/changelog-blind-compare-{light,dark}.png — admin view with system names, grader avatars and awaiting marks beside the member view of the same comparison showing Model A/B */}

  ### Bug Fixes and Improvements

  - **[Compare]** One comparison comment box covers every compared record. Per-record comments sit behind a **Comment on a single record** toggle.
  - **[Compare]** Highlight and annotate text in the **Expected** column on the compare and record pages.
  - **[Compare]** Comparison cards no longer show a **Scores** tab or an **Open** link, so existing grades don't influence preference picks.
  - **[Compare]** Records keep their left/right slots by run order, the **Attachments** tab fills the card with file-type badges, and closing a comparison asks first.
  - **[Records]** A **System** column (hidden by default) on the records table and a `system:` search filter narrow records to one system version. Members see the blind alias.
  - **[Records]** The record page's trace chat can see your own annotations, so you can ask it about what you highlighted.
  - **[Annotations]** Notes support markdown: bold, lists, and strikethrough.
  - **[Clippy]** Clippy shows its reasoning live while it works. Finished reasoning collapses to one line you can expand.
  - **[API]** [Upsert score](/api-reference/upsert-score) accepts an optional `userId` on human metrics to submit a score on behalf of a grader, and every score returned by the API carries the grader's `userId`.
</Update>

<Update label="September 4, 2026">
  ### Code Mode for the hosted MCP server

  The hosted [MCP server](/features/mcp) at `mcp.scorecard.io` now runs `scorecard-ai-mcp` 3.4.0. Its 31 per-endpoint tools are replaced by two: `search_docs` looks up the SDK, and `execute` runs TypeScript your agent writes against the SDK in a sandbox and returns only the result, so a cross-project query over large records completes in one call instead of timing out on multi-megabyte pages.

  ### Bug Fixes and Improvements

  - **[Compare]** Collapse either compared record to a slim rail to give the other the full width. Click the rail to bring it back.
  - **[Compare]** Comparison cards have an **Attachments** tab listing the record's session files. It appears only when the session has files.
  - **[Compare]** The compare search box remembers its query per project, so your filters are back when you return to the page.
  - **[Annotations]** The add-annotation popover opens above the selection when there is no room below, and no longer discards a half-typed note when you click elsewhere. Comment cards can be dragged aside.
  - **[Annotations]** Edit the note on your own annotations in place with the pencil on the comment card, instead of deleting and re-highlighting.
  - **[Records]** Long file names and URLs in outputs wrap instead of pushing the panel behind a horizontal scrollbar.
  - **[Export]** CSV and Excel exports of records and runs include an **Annotations** column.
  - **[Clippy]** Clippy can read a record's trace. Ask why a record failed and it lists the spans and reads the ones it needs.
  - **[Clippy]** A record open in the records list's side panel counts as the page you're on, so Clippy answers about that record.
  - **[Clippy]** Clippy reports metric names instead of version labels when describing scores, renders markdown lists correctly, keeps the earlier conversation when you approve a tool call, and stays pinned to the bottom while a reply streams.
  - **[Models]** The model catalog was refreshed. `gemini-3.8-flash` is the suggested Gemini pick, and Bedrock's suggested Llama is now Llama 4 Maverick.
</Update>

<Update label="August 28, 2026">
  ### Bug Fixes and Improvements

  - **[Compare]** Leave a general comment on either record in the scoring section of the [Compare](/features/a-b-comparison) page. Comments save as annotations on the record and appear in its annotation views.
  - **[Compare]** When two compared records are scored on different exclusive metrics, each cell now names its own metric.
  - **[Clippy]** **Clippy**, the in-app assistant, knows which [record](/features/records) or records list you have open, so you can ask about the record in front of you without giving its id.
</Update>

<Update label="August 21, 2026">
  ### Compare three records at once

  The **Compare** page now allows comparing up to three records, so you can judge three model or prompt versions in one pass instead of running pairwise comparisons back to back. When comparing three records, human float [metrics](/features/metrics) become a 1st/2nd/3rd ranking and human binary metrics become a best-winner pick.

  {/* TODO(screenshot): /images/changelog-compare-three-records-{light,dark}.png — three record cards side by side, shared Expected column leading the row, 1st/2nd/3rd ranking panel below */}
  ...
  {/* TODO(screenshot): /images/changelog-per-model-metrics-{light,dark}.png — preference panel showing a shared pairwise row, a group-paired per-model row, and a one-sided row with the other column empty */}
  ...
  - **[API]** New endpoints for metric groups: [list](/api-reference/list-metric-groups) and [create](/api-reference/create-metric-group) them under a project, then [fetch](/api-reference/get-metric-group), [update](/api-reference/update-metric-group), or [archive](/api-reference/delete-metric-group) one by id.
</Update>

<Update label="August 14, 2026">
  ### Bug Fixes and Improvements

  - **[Records]** Typing a human score and then clicking outside no longer throws the draft away. Reopen the cell and your Pass/Fail pick and reasoning are still in the form.
  - **[Records]** Re-running scoring on a record leaves a completed [human score](/features/metrics) alone, instead of blanking it back to an empty **Add score** cell.
  - **[Records]** The annotations popover shows a highlighted passage as a quote above its note, rather than one run-on block of text. Long lists of annotations scroll instead of running off the screen.
  - **[Models]** `claude-opus-5` and `gemini-3.7-flash` are in the model selector, and both are now the suggested pick for their tier.
  - **[Projects]** The [project](/features/projects) list loads faster, and its testset and metric counts now count only your org's items.
  - **[Records]** Record counts on large projects load in well under a second.
  - **[Sessions]** Attachment lists load faster, most noticeably on sessions with hundreds of files.
</Update>

<Update label="August 7, 2026">
  ### Read and annotate documents on a record

  The record page's **Output** tab renders `.docx`, markdown, and plain-text attachments named in the output inline, with the same highlight-to-annotate layer as the Compare page, so you can review a generated document and comment on it without downloading it. Word files keep their tracked changes and comments.

  {/* TODO(screenshot): /images/changelog-document-review-{light,dark}.png — record Output tab with a redline .docx rendered inline, tracked changes visible, and an annotation highlight with its hover card */}

  ### Bug Fixes and Improvements

  - **[Records]** Any session attachment named in a record's output renders as a clickable chip that opens the preview.
  - **[Records]** A record's **Expected** value sits beside the output in a collapsible column and renders docx, markdown, and text inline.
  - **[Records]** The toolbar wraps and the table scrolls sideways when the details panel is open, instead of hiding behind it.
  - **[Scores]** **Edit** on a completed score lets you change the value and reasoning of any [metric](/features/metrics) after grading, AI and heuristic included.
  - **[Compare]** The compare chat answers at the comparison level and no longer cites individual trace spans.
  - **[Overview]** A **Records** card shows the project's total record count, replacing the Testsets card.
  - **[API]** [Upsert score](/api-reference/upsert-score) accepts a request with no `score`, which queues a human metric for manual grading and leaves it **pending**. AI and heuristic metrics still require a value.
</Update>
...
<Update label="July 24, 2026">
  ...
  A new **Compare** page puts records next to each other in full-width cards so you can read their outputs side by side, with each record's inputs and expected answer alongside. Select text in an output to annotate that passage. Saved annotations stay highlighted inline, and hovering one shows its author and comment.

  <DarkLightImage
    lightSrc="/images/changelog-compare-page-light.png"
    darkSrc="/images/changelog-compare-page-dark.png"
    caption="Comparing two records side by side on the Compare page"
    alt="Screenshot of the Compare page with two record cards side by side on their Output tabs and the shared expected answer in a column between them"
  />

  ### Assign records to teammates

  Assign organization members to a [record](/features/records) from the picker in the record details panel, beside Tags. The records table has an optional **Assignees** column, and typing `assigned.to:` in the search bar narrows the table to one person's records.

  <DarkLightImage
    lightSrc="/images/changelog-record-assignments-light.png"
    darkSrc="/images/changelog-record-assignments-dark.png"
    caption="Assigning an organization member to a record from the details panel"
    alt="Screenshot of the record details panel with an assignee added next to Tags, showing the member's avatar and name"
  />

  ### Import a testset from an agent transcript

  **Import from transcript** on a project's [Testsets](/features/testsets) page takes a pasted or dropped agent-session transcript, such as a Claude Code `session.jsonl`, and pulls out the moments where the agent should have acted differently. Each one becomes a testcase in a new testset, with the prompt as input and the expected behavior as the label. Analysis can take a couple of minutes. Closing the dialog doesn't cancel it, and a toast reports the result when it finishes.

  <DarkLightImage
    lightSrc="/images/changelog-transcript-import-light.png"
    darkSrc="/images/changelog-transcript-import-dark.png"
    caption="Importing a testset from an agent-session transcript"
    alt="Screenshot of the Import from transcript modal showing the file drop zone, a paste field, and an optional guidance input"
  />

  ### Bug Fixes and Improvements

  - **[Records]** Clicking a session attachment opens a preview sheet instead of downloading the file. Images, PDFs, video, audio, markdown, and code render inline, and download is a separate button.
  - **[Records]** Markdown tables in record outputs render as tables instead of raw pipes.
  - **[Onboarding]** The onboarding page shows the organization API key to admins again instead of "No API key found".
  - **[API]** [Record creation](/api-reference/create-record) accepts an optional `sessionId`, so records created through the API or SDK join that session's traces and attachments.
</Update>

<Update label="July 17, 2026">
  ### Session attachments

  You can now attach files to a session and see them next to its [record](/features/records). Attachments appear in a **Files** card in the record details panel and in an **Attachments** tab, and any file an agent read or wrote shows up as a clickable chip inline in the conversation, on the turn that mentions it. Upload runs through the API: declare the file, upload the bytes, then commit.

  <DarkLightImage
    lightSrc="/images/changelog-session-attachments-light.png"
    darkSrc="/images/changelog-session-attachments-dark.png"
    caption="Attachments tab listing files attached to a session"
    alt="Screenshot of the record details panel showing the Attachments tab with quarterly-report.pdf, summary.md, and key-metrics.csv attached to the session"
  />

  ### Records overview widgets

  The [Records](/features/records) page now has a widget board at the top that you can customize. Add a **Volume** widget to see record ingest over time, or a **Metric** widget to track a metric's pass rate — either as a snapshot or a stacked pass/fail/ungraded chart over time. Widgets follow the table's active filters, and your layout is saved per project.

  <DarkLightImage
    lightSrc="/images/changelog-records-widgets-light.png"
    darkSrc="/images/changelog-records-widgets-dark.png"
    caption="Widget board on the Records page with Volume and metric widgets"
    alt="Screenshot of the Records page overview widget board showing a Volume widget alongside Biographical accuracy and Design quality metric widgets"
  />

  ### Skills in Trace Chat

  Trace chat accepts skill attachments. Attach a `.skill` bundle or a plain `.md` file and the chat follows its instructions when it analyzes a trace — for example, checking a run against a bundled rubric. Skills stay attached per project and pull in their reference files as needed.

  <DarkLightImage
    lightSrc="/images/changelog-trace-chat-skills-light.png"
    darkSrc="/images/changelog-trace-chat-skills-dark.png"
    caption="Add skill control in the Trace Chat composer"
    alt="Screenshot of the Trace Chat composer showing the Add skill control above the ask-about-this-trace input"
  />

  ### Bug Fixes and Improvements

  - **[Records]** You can build filters by typing. Enter a field name and a colon (e.g. `status:`) to start a pill, and typed values like metric or creator names resolve to the right filter value when you commit.
  - **[Records]** Fixed the filter popovers overlapping the field dropdown and closing when you clicked their scrollbar.
</Update>

<Update label="July 10, 2026">
  ### Bug Fixes and Improvements

  - **[UI]** Removed the **Temperature**, **Maximum Length**, and **Top P** controls. New configurations run at each provider's defaults; existing versions keep their stored parameters, and you can still set them through the API and SDK.
  - **[Records]** Scoring records with a hidden metric column now reveals that column automatically, with scores filling in as jobs finish.
  - **[Onboarding]** The Claude Agent SDK snippets now include `OTEL_LOG_USER_PROMPTS`, `OTEL_LOG_TOOL_DETAILS`, and `OTEL_LOG_TOOL_CONTENT`.
  - **[API]** Record creation (`POST /v2/runs/:runId/records`) now accepts bodies up to 15MB, and oversized requests return a `413` instead of a `500`.
</Update>

<Update label="May 29, 2026">
  ### 💬 Conversation View Is Now the Default

  The record details page now opens on the threaded **Conversation** view by default, so you land directly on the human-readable agent run instead of the raw span tree. The span tree + details view has been renamed to **Debug** and moved to the end of the tab order to signal it as a developer/inspection tool. Tabs now read **Conversation, Timeline, Chat, Scores, Debug**, and clicking a span from any tab still jumps into Debug with that span selected.

  ### 🔽 Expand All / Collapse All in Conversation View

  Two quality-of-life upgrades for the Conversation view:

  - **Expand all / Collapse all**: A new inline toggle in the Conversation tab header expands or collapses every tool step, compact-turn preview, and long user-turn "Show more" at once. Subagent steps nested inside an `Agent` tool call respect it too, and per-step buttons keep working normally after a bulk action.
  - **Persistent collapse state**: Expanding a step and then switching to Timeline, Chat, or Debug no longer resets everything — what you expanded stays expanded when you come back.
</Update>

<Update label="May 22, 2026">
  ### 📊 Annotations Column

  The Records page now has an opt-in **Annotations** column that surfaces reviewer feedback directly in the table. Each cell shows a compact badge with the total count and per-rating breakdown (👍 / 👎 / 💬). Click the badge to open a popover listing each annotation's author, rating, and comment — no need to open the record details.

  <DarkLightImage
    lightSrc="/images/changelog-annotations-column-light.png"
    darkSrc="/images/changelog-annotations-column-dark.png"
    caption="Annotations column on the Records page"
    alt="Screenshot of the Records page showing the new Annotations column with badges indicating annotation counts on individual records"
  />

  Toggle it on via **Edit Table → Additional → Annotations**. Annotations are joined into the records query in a single round-trip, so adding the column doesn't slow down page loads.

  ### 💬 Trace Comparison in Chat
  ...
  <DarkLightImage
    lightSrc="/images/changelog-trace-chat-compare-light.png"
    darkSrc="/images/changelog-trace-chat-compare-dark.png"
    caption="Comparing two traces side-by-side in the Chat tab"
    alt="Screenshot of the Chat tab on a trace record, showing a side-by-side comparison table of two traces with differences in duration, model, strategy, and final outcome"
  />

  Useful for debugging regressions ("why did this run fail when the previous one passed?") or comparing approaches across projects.
  ...
  - **[AI]** Refreshed the LiteLLM model catalog so `gemini/gemini-3.5-flash` (the current default model) resolves correctly and no longer falls through to an error path for orgs without a custom-model override.
</Update>
...
<Update label="May 8, 2026">
  ...
  <DarkLightImage
    lightSrc="/images/changelog-export-annotations-light.png"
    darkSrc="/images/changelog-export-annotations-dark.png"
    caption="Reorganized Export menu with Records, Annotations, and Traces sections"
    alt="Screenshot of the Export menu showing Export Records as CSV/Excel, Export Annotations as JSON, and Export Traces as JSON"
  />
</Update>
...
<Update label="April 10, 2026">
  ...
  <DarkLightImage
    lightSrc="/images/changelog-records-analysis-light.png"
    darkSrc="/images/changelog-records-analysis-dark.png"
    caption="Records Analysis banner on the Records page"
    alt="Screenshot of the Records Analysis banner showing AI-generated insights with severity indicators and filter actions"
  />
...
<Update label="February 27, 2026">
  ...
  <DarkLightImage
    lightSrc="/images/changelog-traces-chat-tab-light.png"
    darkSrc="/images/changelog-traces-chat-tab-dark.png"
    caption="Chat tab on the Trace details page"
    alt="Screenshot of the Chat tab on a trace record, showing an AI-generated summary and a text input to ask questions about the trace"
  />
...
<Update label="January 30, 2026">
  ...
  <DarkLightImage
    lightSrc="/images/changelog-conversation-tab-light.png"
    darkSrc="/images/changelog-conversation-tab-dark.png"
    caption="Conversation tab showing multi-turn interactions"
    alt="Screenshot of the Conversation tab displaying a chat between a user and an AI agent"
  />
  ...
  <DarkLightImage
    lightSrc="/images/changelog-records-export-light.png"
    darkSrc="/images/changelog-records-export-dark.png"
    caption="Export records as CSV or Excel from the Records page"
    alt="Screenshot of the Records page showing the Export dropdown with CSV and Excel options"
  />
  ...
  <DarkLightImage
    lightSrc="/images/changelog-records-search-light.png"
    darkSrc="/images/changelog-records-search-dark.png"
    caption="Search records by any field value"
    alt="Screenshot of the Records page showing search results filtered by a metadata query"
  />
</Update>
...
<Update label="December 12, 2025">
  ...
  <DarkLightImage
    lightSrc="/images/changelog-records-columns-filters-light.png"
    darkSrc="/images/changelog-records-columns-filters-dark.png"
    caption="Records page with extracted columns, saved layouts, and quick filters"
    alt="Screenshot of the Records page showing extracted input/output columns, Created At / Created By, and the Quick Filters menu"
  />
  ...
  <DarkLightImage
    lightSrc="/images/changelog-projects-topnav-table-light.png"
    darkSrc="/images/changelog-projects-topnav-table-dark.png"
    caption="Project switching via the top navigation"
    alt="Screenshot of the project switcher in the top navigation showing a table of projects"
  />
  ...
  <DarkLightImage
    lightSrc="/images/changelog-metric-templates-light.png"
    darkSrc="/images/changelog-metric-templates-dark.png"
    caption="New metric templates"
    alt="Screenshot of the new metric templates"
    />

  {/* ### SDK tracing wrappers

  We shipped new SDK capabilities to make tracing dramatically easier to adopt in production by automatically instrumenting LLM calls.

  - **LLM Proxy**: Automatically instrument tracing for LLM calls without needing to manually add tracing around every request.
  - **LLM wrap helper**: Wrap popular LLM SDKs (including OpenAI and Anthropic) to auto-instrument tracing in **TypeScript** and **Python**. */}
...
<Update label="December 5, 2025">
  ...
  <DarkLightImage
    lightSrc="/images/changelog-new-playground-light.webp"
    darkSrc="/images/changelog-new-playground-dark.webp"
    caption="New playground"
    alt="Screenshot of the new playground"
  />
  ...
  <DarkLightImage
    lightSrc="/images/changelog-records-page-light.png"
    darkSrc="/images/changelog-records-page-dark.png"
    caption="Records page with filters"
    alt="Screenshot of the records page with filters"
  />
  ...
  <DarkLightImage
    lightSrc="/images/create-project-wizard-light.gif"
    darkSrc="/images/create-project-wizard-dark.gif"
    caption="Create project wizard"
    alt="Screenshot of the create project wizard"
  />
...
<Update label="November 21, 2025">
  ...
  <DarkLightImage
    lightSrc="/images/changelog-monitor-table-light.png"
    darkSrc="/images/changelog-monitor-table-dark.png"
    caption="Table of active monitors on the traces page"
    alt="Screenshot of the table of active monitors on the traces page"
  />
  ...
  <DarkLightImage
    lightSrc="/images/changelog-runs-source-light.png"
    darkSrc="/images/changelog-runs-source-dark.png"
    caption="Filter runs by source on the runs page"
    alt="Screenshot of the runs page with filter by source"
  />
...
<Update label="November 14, 2025">
  ...
  <DarkLightImage
    ...
    caption="Re-run scoring for an existing run"
    alt="Screenshot of the re-run scoring modal for an existing run"
  />
  ...
  <DarkLightImage
    ...
    caption="Create metric modal with Python heuristic code"
    alt="Screenshot of the create metric modal with Python heuristic code defined"
  />
  ...
  <DarkLightImage
    lightSrc="/images/changelog-new-metric-details-page-light.png"
    darkSrc="/images/changelog-new-metric-details-page-dark.png"
    caption="New metric details page"
    alt="Screenshot of the new metric details page"
  />
  ...
  <DarkLightImage
    lightSrc="/images/changelog-metric-template-preview-light.png"
    darkSrc="/images/changelog-metric-template-preview-dark.png"
    caption="Metric template preview"
    alt="Screenshot of the metric template preview"
  />
  ...
  **"Recently used" badge**: Quickly identify metrics that were recently used in a run.
  ...
  <DarkLightImage
    lightSrc="/images/changelog-traces-search-light.png"
    darkSrc="/images/changelog-traces-search-dark.png"
    caption="Traces search with filter options"
    alt="Screenshot of the traces page with search and filter options"
  />
...
<Update label="November 7, 2025">
  ...
  <DarkLightImage
    lightSrc="/images/changelog-trace-details-ux-light.png"
    darkSrc="/images/changelog-trace-details-ux-dark.png"
    caption="Improved trace details UX with a collapsible span tree"
    alt="Screenshot of the trace details page with a collapsible span tree"
  />
  ...
  <DarkLightImage
    lightSrc="/images/changelog-monitor-span-name-regex-light.png"
    darkSrc="/images/changelog-monitor-span-name-regex-dark.png"
    caption="New span name regex filter for monitors"
    alt="Screenshot of the monitor configuration with span name regex filter"
  />
  ...
  <DarkLightImage
    lightSrc="/images/changelog-metric-group-page-light.png"
    darkSrc="/images/changelog-metric-group-page-dark.png"
    caption="Metric groups are now full pages that can be linked to"
    alt="Screenshot of the metric group page"
  />
...
<Update label="October 17, 2025">
  ...
  - **[Traces]** We now create testcases from _any_ OpenTelemetry GenAI span in the tracing page, which extends our support for open standards in LLM observability.
...
<Update label="September 26, 2025">
  ...
  <DarkLightImage
    lightSrc="/images/mcp-evals-results-light.png"
    darkSrc="/images/mcp-evals-results-dark.png"
    caption="MCP Evals results page showing performance metrics and evaluation scores"
    alt="Screenshot of the MCP Evals platform results page"
  />
  ...
  <DarkLightImage
    lightSrc="/images/changelog-new-card-design-light.png"
    darkSrc="/images/changelog-new-card-design-dark.png"
    caption="New metric card design"
    alt="Screenshot of the new metric card design"
  />
  ...
  <DarkLightImage
    lightSrc="/images/changelog-float-score-light.png"
    darkSrc="/images/changelog-float-score-dark.png"
    caption="New float output score of 0.90 along with an explanation."
    alt="Screenshot showing a float score of 0.90 along with an explanation."
  />
...
<Update label="September 12, 2025">
  ...
  <img src="/images/mcp-ecosystem-diagram.svg" alt="MCP Ecosystem Diagram showing how MCP servers connect AI assistants with external tools and services" className="bg-white" />
...
<Update label="August 29, 2025">
  ...
  <DarkLightImage
    lightSrc="/images/changelog/august-29/endpoint-edit-light.png"
    darkSrc="/images/changelog/august-29/endpoint-edit-dark.png"
    caption={null}
    alt="Screenshot of the Edit Endpoint modal"
  />

  <DarkLightImage
    lightSrc="/images/changelog/august-29/kickoff-endpoint-light.png"
    darkSrc="/images/changelog/august-29/kickoff-endpoint-dark.png"
    caption={null}
    alt="Screenshot of Kickoff Run modal with Endpoint tab"
  />
  ...
  <DarkLightImage
    lightSrc="/images/changelog/august-29/onboarding-banner-light.png"
    darkSrc="/images/changelog/august-29/onboarding-banner-dark.png"
    caption={null}
    alt="Screenshot of the onboarding banner"
  />
...
<Update label="August 22, 2025">
  ...
  <DarkLightImage
    lightSrc="/images/trace-overview-create-testcase.png"
    caption={null}
    alt="Screenshot of trace overview with create testcase button"
  />
  ...
  <DarkLightImage
    lightSrc="/images/monitors/trace-to-testcase-select-testset.png"
    caption={null}
    alt="Screenshot of testset selection modal"
  />

  <DarkLightImage
    lightSrc="/images/monitors/trace-to-testcase-fields.png"
    alt="Screenshot of testcase fields auto-populated from trace"
  />
  ...
  <DarkLightImage
    lightSrc="/images/testcase-metadata-field.png"
    alt="Screenshot of add testcase modal showing metadata field"
  />
...
<Update label="August 8, 2025">
  ...
  <DarkLightImage
    lightSrc="/images/sim-agent-details-light.png"
    caption={null}
    alt="Screenshot of Sim Agent configuration"
  />
  ...
  <DarkLightImage
    lightSrc="/images/monitors/online-evaluation-scores.png"
    alt="Screenshot of online evaluation scores in trace details"
  />
  ...
  <DarkLightImage
    lightSrc="/images/monitors/trace-cost-tracking.png"
    alt="Screenshot showing cost tracking in traces list"
  />
...
<Update label="August 1, 2025">
  ...
  <DarkLightImage
    ...
    caption={null}
    alt="Screenshot of the new tracing UI with flame graph visualization"
  />
  ...
  <DarkLightImage
    lightSrc="/images/changelog-run-status-light.png"
    caption={null}
    alt="Screenshot of improved run status with hover explanations"
  />
...
<Update label="May 2, 2025">
  ...
  <iframe
    ...
    src="https://www.youtube.com/embed/aM79Cn6hiCo"
    title="Scorecard MCP Server Overview"
  ...
  <img src="/images/eval_day.png" alt="Scorecard Eval Day event poster" className="max-w-xs" />
