...
<Update label="July 24, 2026">
  ...
  A new **Compare** page puts up to three [records](/features/records) next to each other in full-width cards. Each card has **Output**, **Inputs**, **Expected**, and **Trajectory** tabs, and a tab shows up only when the record has that data. Select text in an output to annotate that passage. Saved annotations stay highlighted inline, and hovering one shows its author and comment.

  {/* TODO(screenshot): /images/changelog-compare-page-light.png + /images/changelog-compare-page-dark.png — Compare page with three record cards side by side, Output tab active, one annotated passage highlighted */}
  ...
  {/* TODO(screenshot): /images/changelog-record-assignments-light.png + /images/changelog-record-assignments-dark.png — records table with the Assignees column on and the assignee picker open in the details panel */}
  ...
  **Import from transcript** on a project's [Testsets](/features/testsets) page takes a pasted or dropped agent-session transcript, such as a Claude Code `session.jsonl`, and pulls out the moments where the agent should have acted differently. Each one becomes a testcase in a new testset, with the prompt as input and the expected behavior as the label. Analysis can take a couple of minutes. Closing the dialog doesn't cancel it, and a toast reports the result when it finishes.
  ...
  {/* TODO(screenshot): /images/changelog-transcript-import-light.png + /images/changelog-transcript-import-dark.png — Import from transcript modal in its input state, with the drop zone and optional guidance field */}
