...
This wrapper leverages OpenTelemetry to capture detailed traces of all AI SDK operations, including inputs, outputs, model parameters, token usage, and latency metrics. All traces are automatically sent to Scorecard where you can analyze performance, debug issues, and monitor production applications.
...
<Steps>
  ...
  <Step title="Get your API key">
    Sign in to [Scorecard](https://app.scorecard.ai) and get your API key from the Settings page.
  ...
  <Step title="View traces in Scorecard">
    Run your application and navigate to the [Tracing page](https://app.scorecard.ai/tracing) in Scorecard to see your traces.
  </Step>
</Steps>
...
Create a single wrapped instance and export it for use throughout your application:
...
export const scorecardAI = wrapAISDK(ai, {
...
```typescript
// app/route.ts
import { scorecardAI } from '@/lib/ai';
import { openai } from '@ai-sdk/openai';

export async function POST(req: Request) {
  const { text } = await scorecardAI.generateText({
    model: openai('gpt-4o-mini'),
...
### 2. Use service versioning

Track performance across releases by setting `serviceVersion`:

```typescript
const aiSDK = wrapAISDK(ai, {
  serviceVersion: process.env.npm_package_version, // Automatically use package.json version
});
```

### 3. Use descriptive service names

Differentiate between services or environments with clear naming:

```typescript
const aiSDK = wrapAISDK(ai, {
  serviceName: `chat-api-${process.env.NODE_ENV}`,
});
```
...
<AccordionGroup>
  <Accordion title="Error: SCORECARD_API_KEY is missing" icon="key">
  ...
  <Accordion title="Error: SCORECARD_PROJECT_ID is missing" icon="folder">
  ...
  <Accordion title="Traces not appearing in Scorecard" icon="magnifying-glass">
  ...
  <Accordion title="Monitor creation failed" icon="triangle-exclamation">
...
<CardGroup cols={2}>
  ...
  <Card title="Monitoring" icon="monitor-heart-rate" href="/features/monitoring">
    Set up production monitoring for your AI applications
  </Card>

  <Card title="Metrics" icon="chart-mixed" href="/features/best-in-class-metrics">
