...
Records created through the [API](/api-reference/create-record) are scored automatically too: creating a record starts scoring for every AI and Heuristic metric on its run. If you push your own scores with [Upsert Score](/api-reference/upsert-score), do so after that automatic scoring finishes — a grade that lands afterwards replaces the pushed score.
