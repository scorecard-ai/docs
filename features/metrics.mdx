...
Deleting a metric removes it from any metric groups and monitors that use it, and scores previously recorded for that metric stop appearing on records and runs in the Scorecard app.

<Note>
  Deletion changes only what the app displays. The API and record exports (CSV and XLSX) continue to return scores from deleted metrics, so integrations and downloaded results still reconcile against your complete scoring history.
</Note>
