...
Deleting a metric removes it from any metric groups and monitors that use it. Scores previously recorded for that metric also stop appearing on records and runs, including in API responses.

<Note>
  Historical score data is retained rather than erased—it is hidden from these views, not destroyed.
</Note>
