...
Scorecard **Monitors** periodically sample recent LLM spans, extract prompts and completions, and automatically score them with your selected metrics. The results appear in the Monitoring dashboard so you can track production quality and detect drift over time.
...
- **Where scores live**: Aggregates are in the Monitoring dashboard; detailed scores are also visible on **Runs & Results** and linked from **Traces**.
...
- Scorecard samples recent AI spans using deterministic, hash-based sampling (stable slices).
- It extracts prompt and completion from span attributes (supports `openinference.*`, `ai.prompt` / `ai.response`, and `gen_ai.*`).
- The monitor creates a new **Run** and turns sampled spans into **Records** that are automatically scored with your selected metrics.
- Results appear in the **Monitors** dashboard and are linked in **Runs & Results** for deeper analysis.
...
- Time-series trends to spot drift.
