...
description: "Evaluate multi-turn conversations with Scorecard."
---

import { DarkLightImage } from '/snippets/dark-light-image.jsx';

Scorecard's multi-turn simulation feature allows you to evaluate the performance of an LLM system in a multi-turn conversation.

Define instructions for your simulated user in Scorecard, then run a simulation on your system using the Scorecard SDK.
...
The `system` parameter is your code under test. It must be a callable that handles conversation turns.
...
```python wrap expandable Example system function
...
To view simulation results, go to the Run's details page. Then, click on a Record ID to go to the record details page. The conversation history between the Sim Agent ("User") and the system under test ("Assistant") will be shown.
...
This Sim Agent will test the system's handling of unusual requests.
