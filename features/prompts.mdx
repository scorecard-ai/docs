...
Prompts in Scorecard are configurable templates that generate inputs for language models. They support dynamic variable substitution and multi-message conversations.
...
Before publishing to production, test a new version with a [Kickoff Run](/features/runs#kickoff-run-from-the-ui) to be confident that it behaves as expected and won't regress on any testcases.
...
<Info>
Ready to evaluate your prompts? Follow the [Runs guide](/features/runs) to test your prompts on Testsets.
</Info>