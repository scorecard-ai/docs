...
<Tabs>
  <Tab title="Claude Agent SDK">
    ...
    <Warning>
      The `env` option replaces the subprocess environment entirely rather than extending it, so always spread `process.env` (or `os.environ`) in as shown below—otherwise the agent loses its API credentials and `PATH`.
    </Warning>
