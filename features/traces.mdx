...
Traces in Scorecard provide detailed visibility into your AI application's execution flow, capturing every step from user input to final output. This powerful observability feature helps you:

- **Debug issues**: Pinpoint exactly where errors occur in complex AI pipelines
- **Monitor performance**: Track latency at each step of your application
- **Analyze costs**: Understand token usage and API costs across different models
- **Improve quality**: Identify bottlenecks and optimization opportunities
...
Each span contains:
- **Timing information**: Start time, duration, and latency
- **Metadata**: Model used, parameters, token counts
...
**Behavioral Errors** (Silent Failures):
- **Off-topic Responses**: AI responds but goes outside intended scope
- **Workflow Interruptions**: Breaking conversation flows or task sequences  
- **Safety Violations**: Responses that violate safety guidelines or policies
- **Hallucinations**: Factually incorrect information presented confidently
- **Context Loss**: Losing track of conversation history or user intent

**Technical Errors**:
- **Model Errors**: Rate limits, timeouts, invalid requests, API failures
- **Application Errors**: Exceptions, crashes, logic errors in your code
- **Validation Errors**: Schema mismatches, type errors, format issues
- **Integration Errors**: Downstream API failures, network connectivity issues
- **Configuration Errors**: Invalid parameters, missing credentials, model misconfigurations
...
- Live error rate tracking across all AI interactions
- Immediate alerts for error spikes or new failure patterns
- Error distribution by type, severity, and affected users

**Failure Pattern Detection**:
- Identify recurring behavioral issues across conversations
- Detect systematic problems with specific prompts or model configurations
- Track error trends over time and deployments

**Root Cause Analysis**:
- Full reasoning chain replay for any failed interaction
- Contextual error explanations with user intent analysis
- Impact assessment showing affected users and conversations
...
<Card title="Monitors" icon="bell" href="/features/monitors">
...
<Card title="API Reference" icon="code" href="/api-reference/traces/send">
