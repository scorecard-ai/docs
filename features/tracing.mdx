...
<Tabs>
  <Tab title="Agent Frameworks">
    For agents and frameworks with built-in OpenTelemetry support (Claude Code, Claude Agent SDK, [Claude Tag](/intro/claude-tag-tracing), OpenAI Agents SDK). No code changes—just set environment variables.
...
Ingested traces appear on the **Records** page alongside records from other sources (API, Kickoff). Each row in the table shows:
