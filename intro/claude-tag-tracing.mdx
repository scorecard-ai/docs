...
Each Claude Tag session becomes one record. Most records are small; a few are very large. That's because Claude Tag runs several kinds of sessions:

| Session | How it starts | What the record looks like |
|---|---|---|
| **Channel session** | Any new top-level message in a channel Claude is in | A few LLM calls, usually ending in a `mcp__slackbot__no_reply_needed` tool call. You get one of these for most messages in the channel, not only the ones addressed to Claude. |
