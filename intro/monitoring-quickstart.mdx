...
<DarkLightImage
  ...
  caption="Create a monitor."
  alt="Create monitor UI."
/>
...
/>
    ...
    **Rule of Thumb: Filtering for GenAI Spans**

    Your monitor must match GenAI spans:
    - Spans need at least one `gen_ai`, `llm.`, or `traceloop` attribute to be captured
    - **Your monitor filter must match these GenAI spans** (not their parent spans)

    **Common pitfall:** Filtering by a parent span name that doesn't have GenAI attributes won't work, even if child spans do.

    **Safe approaches for filtering:**
    1. **Filter by service name** - catches all GenAI spans within that service
    2. **Filter by specific span name** - target the exact span that has GenAI attributes (ideally with proper IO)

    **Quick filter examples:**
    - ✅ `{"serviceName": "my-service"}` - catches all GenAI spans in the service
    - ✅ `{"spanName": "llm-call"}` - catches the specific GenAI span named "llm-call"  
    - ❌ `{"spanName": "workflow"}` - won't work if "workflow" is a parent span without gen_ai attributes
    ...
    **Where prompts & completions come from**
    ...
    Once a GenAI span is captured, IO extraction is forgiving:
    - ✅ **Best case:** Has proper IO attributes (`gen_ai.prompt.*`, `gen_ai.completion.*`) → extracts structured IO
    - ✅ **Fallback case:** Missing IO attributes → automatically uses full trace as input/output
