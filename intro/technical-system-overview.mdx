...
<Tabs>
  ...
  <Tab title="Systems" icon="server">
    ...
    ```typescript
    // Systems are functions that process inputs and return outputs
    type SystemFunction = (inputs: Record<string, any>) => Promise<Record<string, any>>;
    
    // Example: Simple system function
    const jokeBot: SystemFunction = async (inputs) => {
      const response = await openai.chat.completions.create({
        model: "gpt-4o-mini",
        messages: [{
          role: "system", 
          content: "You are a witty joke bot"
        }, {
          role: "user", 
          content: `Tell a joke about ${inputs.topic}`
        }]
      });
      return { joke: response.choices[0].message.content };
    };
    
    // Systems can be passed to run_and_evaluate
    run_and_evaluate(client, {
      project_id: PROJECT_ID,
      testcases: testcases,
      system: jokeBot,
...
<Note>
**Questions?** Engineering support available at team@scorecard.io.
</Note>
