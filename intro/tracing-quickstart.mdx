...
<Note>
  The Python SDK wrappers require the optional `otel` extra, which installs OpenTelemetry. Without it, `wrap()` raises an `ImportError`:

  ```bash
  pip install 'scorecard-ai[otel]'
  ```

  The TypeScript SDK bundles OpenTelemetry, so `npm install scorecard-ai` is all you need.
</Note>

<CodeGroup>
```py Python highlight={4,7,9-13}
# Works with OpenAI and Anthropic - see examples below for more patterns
...
```js JavaScript highlight={3,7,11-15}
// Works with OpenAI and Anthropic - see examples below for more patterns
